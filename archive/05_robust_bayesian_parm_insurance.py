...
for i, product in enumerate(best_products[:5], 1):  # Top 5 products
    ...
    wind_indices = []
    # Create substitute yearset_results from available event data
    available_years = list(range(1980, 2025))  # Simulation years
    yearset_results = {year: list(range(len(event_losses))) for year in available_years}
    
    for year in list(yearset_results.keys())[n_train:]:  # Validation years
        if event_impacts_dict:
            # Use maximum wind speed as simplified index
            year_wind_speeds = []
            events = yearset_results.get(year, [])
            for event_id in events[:3]:  # Top 3 events per year
                if str(event_id) in event_impacts_dict:
...
model_spec = ModelSpec(
    ...
    # contamination_distribution=ContaminationDistribution.CAUCHY,  # Skip for now
    model_name="hierarchical_epsilon_contamination_integrated"
)

print(f"   污染分布類型: north_carolina_tc")
